...
| `extraUsers` | `[]string` | No | `[]` | Additional login roles to provision in each restore, beyond the analytics user. Each becomes a `LOGIN SUPERUSER` role with an operator-generated password stored in a per-user Secret `<replica>-user-<slugged name>-creds` (owned by the replica, labelled `pgro.bes.au/component=extra-user-creds`, and carrying the unslugged role name under `username`). Provisioned once at restore initialisation and never dropped; a name matching `analyticsUsername` is ignored. |
