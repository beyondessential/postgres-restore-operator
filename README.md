...
| `analyticsUsername` | `string` | No | `"analytics"` | Username created for analytics connections. Attributes, memberships and password are reset to a fixed state on every restore, so a name colliding with a role in the source cluster doesn't carry production's privileges forward; `readOnly` then decides whether the role gets `pg_read_all_data` or `SUPERUSER`. |
| `extraUsers` | `[]string` | No | `[]` | Additional login roles to provision in each restore, beyond the analytics user. Each name is reset to a `LOGIN` role with no privileges of its own and read-only sessions, so it reads nothing but the system catalogues until something grants it access, and those grants must be reapplied per restore. Declaring any extra user also revokes the `public` schema from the `PUBLIC` pseudo-role in every database of that restore, since postgres has no per-role deny and that is the only way to keep an ungranted role out of it; the analytics user reaches schemas through `pg_read_all_data` or `SUPERUSER` and is unaffected. The password is operator-generated and stored in a per-user Secret `<replica>-user-<slugged name>-creds` (owned by the replica, labelled `pgro.bes.au/component=extra-user-creds`, and carrying the unslugged role name under `username`). Provisioned once at restore initialisation and never dropped; a name matching `analyticsUsername` is ignored. |
