...
| `migrationResult` | `MigrationResult` | What the migrations did: `totalElapsedSeconds`, `failedMigration` (absent on success), `dataBytesBefore`/`dataBytesAfter`, and `timings` (one `{ name, elapsedSeconds }` per migration, in the order they ran). Read out of Tamanu's own `logs.migrations` audit table. |
| `conditions` | `[]Condition` | Standard Kubernetes conditions. |
