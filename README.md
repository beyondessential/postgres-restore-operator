...
| `analyticsUsername` | `string` | No | `"analytics"` | Username created for analytics connections. Attributes, memberships and password are reset to a fixed state on every restore, so a name colliding with a role in the source cluster doesn't carry production's privileges forward; `readOnly` then decides whether the role gets `pg_read_all_data` (plus an explicit `EXECUTE` grant on every function, which `pg_read_all_data` doesn't cover) or `SUPERUSER`. |
| `extraUsers` | `[]ExtraUserSpec` | No | `[]` | Additional login roles to provision in each restore, beyond the analytics user. See [ExtraUserSpec](#extrauserspec) below. Every restore also runs a database-wide lockdown, regardless of whether it declares any extra users: it revokes `PUBLIC`'s privileges on every non-system schema, table/view/sequence, and function, and on default privileges for objects not yet created, since postgres has no per-role deny and that's the only way to keep an ungranted role out of them; and for the analytics user plus each extra user it reassigns any objects they own (to `postgres`, keeping the objects) and drops any direct grants made to them, so a name colliding with a source-cluster role doesn't keep that role's ownership or grants either. The analytics user is unaffected — it reaches its access through `pg_read_all_data`/`SUPERUSER` and an explicit function grant, never through `PUBLIC` or object ownership. |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `name` | `string` | Yes | The role name to provision. A name matching `analyticsUsername` is ignored — that role is already provisioned. |
| `schemas` | `[]string` | No | Schemas this user is granted `USAGE` + `SELECT` on (current tables/sequences, plus default privileges covering tables the persistent-schemas migration creates afterwards), in whichever databases they exist in. Empty (the default) means the role gets no access of its own — it can connect and read the system catalogues, and nothing else, until something grants it more. Those grants have to be reapplied per restore, since each one is a fresh database. |

Each role is reset to a fixed state on every restore — attributes (`LOGIN`, none of `SUPERUSER`/`CREATEDB`/`CREATEROLE`/`REPLICATION`/`BYPASSRLS`), role memberships, password — whether it's created fresh or collides with a role restored from the source cluster, so a colliding name never keeps production's attributes. Its sessions are pinned read-only regardless of the replica's own `readOnly` setting. The password is operator-generated and stored in a per-user Secret `<replica>-user-<slugged name>-creds` (owned by the replica, labelled `pgro.bes.au/component=extra-user-creds`, and carrying the unslugged role name under `username`), created before the restore's initialisation runs. Provisioned once and never dropped.
...
| `migrationResult` | `MigrationResult` | What the migrations did: `totalElapsedSeconds`, `failedMigration` (absent on success; `unknown` when the target version does not record the migration it stopped at), `error` (only on failure: Tamanu's structured error where the version records one, otherwise the tail of the job's log with everything that could quote row contents redacted), `dataBytesBefore`/`dataBytesAfter`, and `timings` (one `{ name, elapsedSeconds }` per migration, in the order they ran). Read out of Tamanu's own `logs.migrations` audit table. |
| `conditions` | `[]Condition` | Standard Kubernetes conditions. |
