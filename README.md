...
| `migrationResult` | `MigrationResult` | What the migrations did: `totalElapsedSeconds`, `failedMigration` (absent on success; `unknown` when the target version does not record the migration it stopped at), `error` (only on failure: Tamanu's structured error where the version records one, otherwise the tail of the job's log; carries the row postgres refused, so the data can be fixed before the upgrade is retried), `dataBytesBefore`/`dataBytesAfter`, and `timings` (one `{ name, elapsedSeconds }` per migration, in the order they ran). Read out of Tamanu's own `logs.migrations` audit table. |
| `conditions` | `[]Condition` | Standard Kubernetes conditions. |
