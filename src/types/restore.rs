...
pub struct PostgresPhysicalRestoreStatus {
	...
	/// How many times this restore's reporting-schema build has been gone
	/// round: a setup that failed, or a Job that ended holding no schema. The
	/// record a build ends with is final, so a run of these is what settles one
